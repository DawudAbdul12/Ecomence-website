...
<?php
                             ...
                             // This first query is just to get the total count of 
                               $status  = "Yes";
					           $tb_select = "SELECT COUNT(id) FROM gallery_tb WHERE published = '{$status}'";
		...
		<div class="container-fluid">
			...
			<?php
				...
				// This is your query again, it is for grabbing just one page worth of rows by applying $limit
				              // $limit = "limit 0,9";
				               $status = "Yes";
				               $tb_select = "SELECT * FROM gallery_tb WHERE published = '{$status}' ORDER BY id DESC $limit";
							   $result = mysqli_query($connect,$tb_select);
							   if(!$result){
							   die ("SELECT TABLE FAILED".mysqli_error());
							   }
							   while($row = mysqli_fetch_array($result)){
							    $encryp_gallery_id = my_simple_crypt($row['0'], 'e' );
							   ?>
				<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12 no-pad">
					<a href="gallery.php?gallery_id=<?php echo $encryp_gallery_id;?>"><img src="<?php echo $row['image']?>" class="img-responsive"></a>
				</div>
