...
<?php require_once("includes/function.php");?>
							...
							<?php
											 ...
											 //  $product_id =  $_SESSION['product_id'];
											   if(isset($attri_id)){
												$tb_select = "SELECT COUNT(id) FROM product_tb WHERE publish = '{$status}'  AND attribute = {$attri_id} AND sale_price != {$sales} ";
												}elseif(isset($brnd_id)){
												$tb_select = "SELECT COUNT(id) FROM product_tb WHERE publish = '{$status}'  AND brand = {$brnd_id} AND sale_price != {$sales} ";
												}elseif(isset($category_id)){
												$tb_select = "SELECT COUNT(id) FROM product_tb WHERE publish = '{$status}' AND category = {$category_id} AND sale_price != {$sales} ";
												}else{
												$tb_select = "SELECT COUNT(id) FROM product_tb WHERE publish = '{$status}'  AND sale_price != {$sales} ";
												}
				...
				if($last != 1){
					...
					if ($pagenum > 1) {
						...
						$paginationCtrls .= '<a href="'.$_SERVER['PHP_SELF'].'?pn='.$previous.'&attr='.$attri_id.'"></a>';
						
						}elseif(isset($brnd_id)){
						 $paginationCtrls .= '<a href="'.$_SERVER['PHP_SELF'].'?pn='.$previous.'&brnd='.$brnd_id.'"></a>';
						}elseif(isset($category_id)){
						$paginationCtrls .= '<a href="'.$_SERVER['PHP_SELF'].'?category='.$category_id.'&pn='.$previous.'"></a>';
						...
						for($i = $pagenum-4; $i < $pagenum; $i++){
							if($i > 0){
							     if(isset($attri_id)){ 
							   $paginationCtrls .= '<a href="'.$_SERVER['PHP_SELF'].'?pn='.$i.'&attr='.$attri_id.'" >'.$i.'</a> ';
						      }elseif(isset($brnd_id)){
							
								$paginationCtrls .= '<a href="'.$_SERVER['PHP_SELF'].'?pn='.$i.'&brnd='.$brnd_id.'" >'.$i.'</a> ';
					...
					if ($pagenum != $last) {
						...
						$paginationCtrls .= '<a href="'.$_SERVER['PHP_SELF'].'?pn='.$next.'&attr='.$attri_id.'"></a>';
						
						}elseif(isset($brnd_id)){
						
						 $paginationCtrls .= '<a href="'.$_SERVER['PHP_SELF'].'?pn='.$next.'&brnd='.$brnd_id.'"></a>';
						 
						}elseif(isset($category_id)){
						
						$paginationCtrls .= '<a href="'.$_SERVER['PHP_SELF'].'?category='.$category_id.'&pn='.$next.'"></a>';
                        ...
                        <div class="col-md-3 col-sm-4">
                            <div class="sideBar">
                                <div class="comapre-product sideBox">
                                    ...
                                    <div class="heading">All Categories</a></div>
                                    <div class="innerContent">
                                         ...
                                         <div class="filterBox">
											...
											<?php 
											    $parnt = 0;
											   $tb_select = "SELECT * FROM category_tb WHERE parent = {$parnt}  ";
											   $result = mysqli_query($connect,$tb_select);
											   if(!$result){
											   die ("SELECT TABLE FAILED".mysqli_error());
											   }
											...
											?>
											<li><a href="sales_list.php?category=<?php echo $row_cat['0']; ?>"><strong><?php echo $row_cat['1'];?><span></span></strong></a></li>
											<?php }else{ ?>
											<li><a href="sales_list.php?category=<?php echo $row_cat['0']; ?>"><?php echo $row_cat['1'];?><span></span></a></li>
											<?php 
											}// close tag of the IF statement
											
											
											   $tb_select_sub = "SELECT * FROM category_tb WHERE parent = {$parent_id} ";
											   $result_sub = mysqli_query($connect,$tb_select_sub);
											   if(!$result_sub){
											   die ("SELECT TABLE FAILED".mysqli_error());
											   }
											   while($row_cat_sub = mysqli_fetch_array($result_sub)){
											  
											   if($row_cat_sub['0'] == $category_id){
											?>
											<li><a href="sales_list.php?category=<?php echo $row_cat_Sub['0']; ?>"><strong>- <?php echo $row_cat_sub['1'];?><span></span></strong></a></li>
											<?php }else{ ?>
											<li><a href="sales_list.php?category=<?php echo $row_cat_sub['0']; ?>">- <?php echo $row_cat_sub['1'];?><span></span></a></li>
											<?php 
											}// close tag of the IF statement
											
											}// CLOSE OF WHILE LOOP FOR SUB CATEGORIES
									...
									//  STATEMENT FOR FEATURED PRODUCT 
									      $attr_id = array();
										   $tb_select_cat_attribute_tb = "SELECT * FROM category_attribute_tb ORDER BY ID DESC " ;
										...
										echo '<strong><li style=""><a href="'.$_SERVER['PHP_SELF'].'?attr='.$row_attr_tb['0'].'" >'.$row_attr_tb['name'].'<span ></span></a></li></strong>';
										   }else{
										   
										   echo '<li><a href="'.$_SERVER['PHP_SELF'].'?attr='.$row_attr_tb['0'].'" >'.$row_attr_tb['name'].'<span ></span></a></li>';
									...
									//  STATEMENT FOR FEATURED PRODUCT 
									   $cat_id = array();
										   $tb_select_cat_brand_tb = "SELECT * FROM category_brand_tb  ORDER BY id DESC " ;
										   ...
										   $tb_select_brand_tb = "SELECT * FROM brand_tb WHERE ID IN (".implode(',',$cat_id).") ORDER BY id DESC " ;
										   ...
										   echo '<strong><li style=""><a href="'.$_SERVER['PHP_SELF'].'?brnd='.$row_brand_tb['0'].'" >'.$row_brand_tb['name'].'<span ></span></a></li></strong>';
										   }else{
										    echo '<li style=""><a href="'.$_SERVER['PHP_SELF'].'?brnd='.$row_brand_tb['0'].'" >'.$row_brand_tb['name'].'<span ></span></a></li>';
                           ...
                           <div class="col-md-8">
                            ...
                            <div class="row products">
							  ...
							  <?php
							   ...
							   if(isset($attri_id)){
							    $tb_select_list = "SELECT * FROM product_tb  WHERE  attribute = {$attri_id} AND sale_price != {$sales} ORDER by id DESC {$limit} ";
	               
							   }elseif(isset($brnd_id)){
							      $tb_select_list = "SELECT * FROM product_tb  WHERE brand = {$brnd_id} AND sale_price != {$sales} ORDER by id DESC {$limit} ";
							   }elseif(isset($category_id)){
							   
							   $tb_select_list = "SELECT * FROM product_tb  WHERE category = {$category_id} AND sale_price != {$sales} ORDER by id DESC {$limit} ";
							  
							   }else{
							    $tb_select_list = "SELECT * FROM product_tb  WHERE  sale_price != {$sales} ORDER by id DESC {$limit} ";
							   }
